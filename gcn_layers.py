...
class GraphConv(nn.Module):
    ...
    def forward(self, input):
        ...
        b, cv, t
        '''
        # Add these lines before matrix multiplication
        print(f"Input shape: {input.shape}")
        print(f"Weight shape: {self.weight.shape}")

        # If needed, reshape input to match the weight's first dimension
        if input.shape[1] != self.weight.shape[0]:
            input = input.view(-1, self.weight.shape[0])  # Adjust this according to your expected dimensions
...
class GraphConvBlock(nn.Module):
    ...
    def forward(self, input):
        ...
        if self.resual:
            ...
            return x
# ResGCB-> T-ResGCB
# SEW-ResNet SEW->Spiking Element-Wise
# EWR -> Element-Wise Recurrent
# class EWR-ResNet(nn.Module):
