...
class LN(nn.Module):
    def __init__(self, dim, epsilon=1e-5):
        ...
        self.alpha = nn.Parameter(torch.ones([1, 28, 15]), requires_grad=True)#[1, 15, 1] [1, dim, 1]
        self.beta = nn.Parameter(torch.zeros([1, 28, 15]), requires_grad=True)
    ...
    def forward(self, x):
       ...
       # print(f"y shape: {y.shape}, alpha shape: {self.alpha.shape}, beta shape: {self.beta.shape}")
...
class LN_v2(nn.Module):
    def __init__(self, dim, epsilon=1e-5):
        ...
        self.alpha = nn.Parameter(torch.ones([1, 28, 15]), requires_grad=True)
        self.beta = nn.Parameter(torch.zeros([1, 28, 15]), requires_grad=True)
